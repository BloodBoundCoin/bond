...
Object blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zBLOOD_BOUND_MASTERNODEObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zBLOOD_BOUND_MASTERNODEObj.emplace_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.emplace_back(Pair("zBONDsupply", zBLOOD_BOUND_MASTERNODEObj));
...
Value getrawmempool(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in BLOOD_BOUND_MASTERNODE\n"
...
Value gettxout(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of BLOOD_BOUND_MASTERNODE addresses\n"
            "     \"BLOOD_BOUND_MASTERNODEaddress\"   	 	(string) BLOOD_BOUND_MASTERNODE address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
