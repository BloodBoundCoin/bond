...
#include "zBLOOD_BOUND_MASTERNODEcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBOND ought to be enough for anybody." - Bill Gates, 2017
    ui->zBONDpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBONDSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    // BLOOD_BOUND_MASTERNODE settings
    QSettings settings;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBOND->setEnabled(false);
        ui->pushButtonMintzBOND->setToolTip(tr("zBOND is currently disabled due to maintenance."));

        ui->pushButtonSpendzBOND->setEnabled(false);
        ui->pushButtonSpendzBOND->setToolTip(tr("zBOND is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBONDpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBOND_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBOND is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBOND...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBOND in ") +
...
void PrivacyDialog::on_pushButtonSpendzBOND_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBOND is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBOND
        sendzBOND();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBOND
    sendzBOND();
}

void PrivacyDialog::on_pushButtonZBLOOD_BOUND_MASTERNODEControl_clicked()
{
    ZBLOOD_BOUND_MASTERNODEControlDialog* zBLOOD_BOUND_MASTERNODEControl = new ZBLOOD_BOUND_MASTERNODEControlDialog(this);
    zBLOOD_BOUND_MASTERNODEControl->setModel(walletModel);
    zBLOOD_BOUND_MASTERNODEControl->exec();
}

void PrivacyDialog::setZBLOOD_BOUND_MASTERNODEControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBLOOD_BOUND_MASTERNODESelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBOND()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BLOOD BOUND MASTERNODE Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBONDpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBOND is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BOND </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBOND</b>";
    ...
    // use mints from zBLOOD_BOUND_MASTERNODE selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBLOOD_BOUND_MASTERNODEControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBLOOD_BOUND_MASTERNODEControlDialog::GetSelectedMints();
    }
    ...
    // Clear zBLOOD_BOUND_MASTERNODE selector in case it was used
    ZBLOOD_BOUND_MASTERNODEControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBLOOD BOUND MASTERNODE Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BLOOD BOUND MASTERNODE, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBLOOD BOUND MASTERNODE Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBOND </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBOND "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBOND "));
    ui->labelzBONDAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBONDSyncStatus->setVisible(fShow);
}
