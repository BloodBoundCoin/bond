...
#define URI_SCHEME "BLOOD_BOUND_MASTERNODE"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BLOOD BOUND MASTERNODE address (e.g. %1)").arg("bL8NFhbfhFuHyCTZfaAe1nkFVno4me4HUL"));
#endif
...
{
    // return if URI is not valid or is no BLOOD_BOUND_MASTERNODE: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::BOND, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert BLOOD_BOUND_MASTERNODE:// to BLOOD_BOUND_MASTERNODE:
    //
    //    Cannot handle this later, because BLOOD_BOUND_MASTERNODE:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BOND, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open BLOOD_BOUND_MASTERNODE.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BLOOD_BOUND_MASTERNODE.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BLOOD_BOUND_MASTERNODE.lnk
...
{
    return GetAutostartDir() / "BLOOD_BOUND_MASTERNODE.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a BLOOD_BOUND_MASTERNODE.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BLOOD_BOUND_MASTERNODE\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the BLOOD_BOUND_MASTERNODE app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add BLOOD_BOUND_MASTERNODE app to startup item list
