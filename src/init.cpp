...
// Copyright (c) 2018 The BLOOD_BOUND_MASTERNODE developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/BLOOD_BOUND_MASTERNODE-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("BLOOD_BOUND_MASTERNODE-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "BOND.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "BOND.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BOND and zBOND money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), MASTERNODE_PORT, 12007));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in BOND/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in BOND/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, BLOOD_BOUND_MASTERNODE, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in BOND/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all BLOOD BOUND MASTERNODE specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:11007"));
    ...
    strUsage += HelpMessageOpt("-backupzBLOOD_BOUND_MASTERNODE=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBLOOD_BOUND_MASTERNODE minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeBLOOD_BOUND_MASTERNODEamount=<n>     " + strprintf(_("Keep N BOND anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 12007, 38843));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The BLOOD BOUND MASTERNODE Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("BLOOD_BOUND_MASTERNODE-loadblk");
...
/** Sanity checks
 *  Ensure that BLOOD_BOUND_MASTERNODE is running in a usable environment with all
...
/** Initialize BLOOD_BOUND_MASTERNODE.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BLOOD BOUND MASTERNODE Core is shutting down."));
...
#endif
    // Make sure only a single BLOOD_BOUND_MASTERNODE process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BLOOD BOUND MASTERNODE Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("BLOOD BOUND MASTERNODE version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // BLOOD_BOUND_MASTERNODE: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZBONDMinted();
                        RecalculateZBONDSpent();
                    }
                    RecalculateBONDSupply(1);
                }
                ...
                // BLOOD_BOUND_MASTERNODE: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BLOOD BOUND MASTERNODE Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart BLOOD BOUND MASTERNODE Core to complete") << "\n";
        ...
        bool fEnableZBLOOD_BOUND_MASTERNODEBackups = GetBoolArg("-backupzBLOOD_BOUND_MASTERNODE", true);
        pwalletMain->setZBLOOD_BOUND_MASTERNODEAutoBackups(fEnableZBLOOD_BOUND_MASTERNODEBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeBLOOD_BOUND_MASTERNODEAmount = 2;
...
//    }
//
//    nAnonymizeBLOOD_BOUND_MASTERNODEAmount = GetArg("-anonymizeBLOOD_BOUND_MASTERNODEamount", 0);
//    if (nAnonymizeBLOOD_BOUND_MASTERNODEAmount > 999999) nAnonymizeBLOOD_BOUND_MASTERNODEAmount = 999999;
//    if (nAnonymizeBLOOD_BOUND_MASTERNODEAmount < 2) nAnonymizeBLOOD_BOUND_MASTERNODEAmount = 2;
    ...
    LogPrintf("Anonymize BLOOD BOUND MASTERNODE Amount %d\n", nAnonymizeBLOOD_BOUND_MASTERNODEAmount);
    ...
    /* Denominations
       ...
       For example:
       1BOND+1000 == (.1BOND+100)*10
       10BOND+10000 == (1BOND+1000)*10
    */
