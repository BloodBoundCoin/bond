...
*Setup instructions for a gitian build of BLOOD_BOUND_MASTERNODE using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BLOOD_BOUND_MASTERNODE
...
to BLOOD_BOUND_MASTERNODE-crypto.com.
...
- [Building BLOOD_BOUND_MASTERNODE](#building-BLOOD_BOUND_MASTERNODE)
...
Clone the git repositories for BLOOD_BOUND_MASTERNODE and gitian and then checkout the BLOOD_BOUND_MASTERNODE version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the BLOOD_BOUND_MASTERNODE repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building BLOOD_BOUND_MASTERNODE
----------------

To build BLOOD_BOUND_MASTERNODE (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the BLOOD_BOUND_MASTERNODE repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/BLOOD_BOUND_MASTERNODE/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/BLOOD_BOUND_MASTERNODE
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/BLOOD_BOUND_MASTERNODE.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit BLOOD_BOUND_MASTERNODE=${COMMIT} --url BLOOD_BOUND_MASTERNODE=${URL} ../BLOOD_BOUND_MASTERNODE/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit BLOOD_BOUND_MASTERNODE=${COMMIT} --url BLOOD_BOUND_MASTERNODE=${URL} ../BLOOD_BOUND_MASTERNODE/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit BLOOD_BOUND_MASTERNODE=${COMMIT} --url BLOOD_BOUND_MASTERNODE=${URL} ../BLOOD_BOUND_MASTERNODE/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/BLOOD_BOUND_MASTERNODE-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/BLOOD_BOUND_MASTERNODE-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/BLOOD_BOUND_MASTERNODE-build.assert
```
